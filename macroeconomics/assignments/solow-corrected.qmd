...
library(scales)
 ...
 ggplot(base_df, aes(x = k)) +
  ...
  geom_line(aes(y = deprec, color = "Depreciation: delta k"), size = 1.1, linetype = "dashed") +
  geom_point(data = ss_df, aes(y = y), color = "black", size = 3) +
  annotate("text", x = ss_k + 6, y = ss_y, label = "Steady State (k*)", hjust = 0) +
  scale_color_manual(values = c("Investment: s k^{0.5}" = "#2C7FB8", "Depreciation: delta k" = "#D95F02")) +
...
plot_df <- dplyr::bind_rows(
  ...
  tibble(k = k, y = deprec, series = "Depreciation (delta k)")
)

 ggplot(plot_df, aes(x = k, y = y, color = series)) +
  geom_line(size = 1.1) +
  geom_point(data = ss_df2, aes(x = k, y = y), color = "black", size = 3, inherit.aes = FALSE) +
  geom_text_repel(data = ss_df2, aes(x = k, y = y, label = label), inherit.aes = FALSE, nudge_x = 5, size = 4) +
  scale_color_manual(values = c("Investment (s=0.3)" = "#2C7FB8", "Investment (s=0.5)" = "#1A9850", "Depreciation (delta k)" = "#D95F02")) +
...
ss_dep_df <- tibble(
  label = c("Old SS (delta=0.05)", "New SS (delta=0.10)"),
...
plot_df_dep <- dplyr::bind_rows(
 ...
 ggplot(plot_df_dep, aes(x = k, y = y, color = series)) +
  ...
  geom_text_repel(data = ss_dep_df, aes(x = k, y = y, label = label), inherit.aes = FALSE, nudge_x = 5, size = 4) +
